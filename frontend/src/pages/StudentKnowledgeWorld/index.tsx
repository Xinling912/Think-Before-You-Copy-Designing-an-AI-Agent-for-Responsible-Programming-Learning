...
import {
  ...
  englishVisibleText,
} from '../../components/KGWorld/language';
...
export default function StudentKnowledgeWorldPage() {
  ...
  return (
    <main className="rea-page kg-explorer-page student-knowledge-world-page">
      <section className="rea-header kg-explorer-header">
        <div>
          <div className="rea-kicker">Learning Chat</div>
          <h1 className="rea-title">Knowledge World</h1>
        </div>
        <div className="kg-explorer-header-actions">
          <Button href="/" aria-label="Back to Learning Chat" icon={<ArrowLeftOutlined />}>
      ...
      <section className="kg-explorer-viewport" aria-label="Knowledge World">
        {loadState.kind === 'request-error' ? (
          <StudentLoadError detail={loadState.detail} onReload={loadOverview} />
        ) : null}
        {loadState.kind === 'integrity-error' ? (
          <StudentIntegrityError errors={loadState.errors} onReload={loadOverview} />
        ) : null}
        {overview && canvasUnsupported ? <StudentReadOnlyFallback overview={overview} /> : null}
        ...
        {skeletonVisible ? <StudentKnowledgeWorldSkeleton exiting={skeletonExiting} /> : null}
      </section>
    </main>
  );
}

function StudentKnowledgeWorldSkeleton({ exiting }: { exiting: boolean }) {
  return (
    <div
      className={`kg-world-skeleton${exiting ? ' is-exiting' : ''}`}
      data-testid="student-kg-world-skeleton"
      aria-label="Loading Knowledge World"
      aria-hidden={exiting}
    >
      <div className="kg-world-skeleton__regions">
        {Array.from({ length: 6 }, (_, index) => (
          <span key={index} data-testid="student-kg-skeleton-region" />
        ))}
      </div>
      <div className="kg-world-skeleton__nodes">
        {Array.from({ length: 24 }, (_, index) => (
          <span key={index} data-testid="student-kg-skeleton-node" />
        ))}
      </div>
    </div>
  );
...
function StudentLoadError({ detail, onReload }: { detail: string; onReload: () => Promise<void> }) {
  const visibleDetail = englishVisibleText(detail, 'Request failed.');
  return (
...
function StudentIntegrityError({ errors, onReload }: { errors: string[]; onReload: () => Promise<void> }) {
  const visibleErrors = [...new Set(errors
    .filter(Boolean)
    .map((error) => englishVisibleText(error, 'Knowledge graph data failed validation.')))];
  return (
    <section className="kg-world-status-panel" role="alert">
...
function StudentReadOnlyFallback({ overview }: { overview: KGOverview }) {
  const englishOverview = useMemo(() => englishKnowledgeOverview(overview), [overview]);
  return (
    <section className="kg-readonly-fallback" data-testid="student-kg-readonly-fallback">
      <header>
        <p className="rea-kicker">Read-only Knowledge World</p>
        <h2>Canvas is unavailable in this browser. The complete graph remains accessible below.</h2>
      </header>
      <section aria-labelledby="student-kg-fallback-categories">
        <h2 id="student-kg-fallback-categories">Knowledge categories</h2>
        <div className="kg-readonly-fallback__categories">
          {englishOverview.categories.map((category) => (
      ...
      <details open>
        <summary>Nodes · {englishOverview.counts.nodes}</summary>
        <ul className="kg-readonly-fallback__records">
          {englishOverview.nodes.map((node) => (
            <li key={node.node_id}>
      ...
      <details>
        <summary>Learning paths · {englishOverview.counts.paths}</summary>
        <ul className="kg-readonly-fallback__records">
          {englishOverview.paths.map((path) => (
            <li key={path.path_id}>
